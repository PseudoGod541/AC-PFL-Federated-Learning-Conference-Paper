...
def create_keras_model(input_shape):
    """Creates a robust, separable LSTM model."""
    base_model = tf.keras.models.Sequential([
        tf.keras.layers.LSTM(64, input_shape=input_shape, return_sequences=True),
        tf.keras.layers.BatchNormalization(), tf.keras.layers.Dropout(0.3),
        tf.keras.layers.LSTM(32), tf.keras.layers.BatchNormalization(),
...
class CmapssClient(fl.client.NumPyClient):
    def __init__(self, model, base_model, head_model, x_train, y_train, x_val, y_val):
    ...
    def fit(self, parameters, config):
        ...
        return self.base_model.get_weights(), len(self.x_train), {}
...
def run_client(cid: int, seed: int):
    ...
    model, base_model, head_model = create_keras_model(input_shape)
    flower_client = CmapssClient(model, base_model, head_model, x_train, y_train, x_val, y_val)
    fl.client.start_client(server_address="127.0.0.1:8080", client=flower_client.to_client())

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Flower FedPer Simulation")
