...
# --- CHANGE 1: Use the standard FedAvg model architecture ---
def create_keras_model(input_shape):
    """Creates a robust, non-separable LSTM model."""
...
# --- CHANGE 2: CmapssClient now handles a single model (like FedAvg) ---
class CmapssClient(fl.client.NumPyClient):
    def __init__(self, model, x_train, y_train, x_val, y_val, cluster_id):
        self.model = model
        self.x_train, self.y_train, self.x_val, self.y_val = x_train, y_train, x_val, y_val
        self.cluster_id = cluster_id
        
    def get_parameters(self, config): 
        # Send the full model's weights
        return self.model.get_weights()

    def fit(self, parameters, config):
        # Set the full model's weights
        ...
        return self.model.get_weights(), len(self.x_train), {"cluster_id": self.cluster_id}
...
# --- ACStrategy (No changes needed, it's already perfect for this) ---
...
# --- Main Logic ---
def run_server(num_clients: int, log_dir: str):
    ...
    # --- CHANGE 3: Initialize with the standard model ---
    initial_model = create_keras_model(input_shape=(30, NUM_FEATURES))
    ...
    history = fl.server.start_server(server_address="0.0.0.0:8080", config=fl.server.ServerConfig(num_rounds=NUM_ROUNDS), strategy=strategy)
    if history.metrics_distributed.get('mae'):
        best_mae = min([mae for _, mae in history.metrics_distributed['mae']])
        results_path = os.path.join(log_dir, "final_results.json")
        with open(results_path, "w") as f:
            json.dump({"best_mae": best_mae}, f)
        print(f"✅ Simple CFL results saved to {results_path}")
...
def run_client(cid: int, seed: int):
    ...
    # --- CHANGE 4: Create and use the standard model ---
    model = create_keras_model((X.shape[1], X.shape[2]))
    client = CmapssClient(model, x_train, y_train, x_val, y_val, cluster_id)
    fl.client.start_client(server_address="127.0.0.1:8080", client=client.to_client())

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simple CFL Simulation")
    parser.add_argument("--mode", required=True, choices=["server", "client"])
    parser.add_argument("--cid", type=int, help="Client ID.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed.")
    parser.add_argument("--num_clients", type=int, default=4, help="Total clients.")
    parser.add_argument("--log_dir", type=str, default=".", help="Directory to save results.")
    args = parser.parse_args()
    if args.mode == "server":
        run_server(args.num_clients, args.log_dir)
